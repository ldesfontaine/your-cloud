...
try {
    ...
    $packageLockSha256 = (
        Get-FileHash -Algorithm SHA256 -LiteralPath (Join-Path $app "package-lock.json")
    ).Hash.ToLowerInvariant()
    $cargoLockSha256 = (
        Get-FileHash -Algorithm SHA256 -LiteralPath (Join-Path $app "src-tauri\Cargo.lock")
    ).Hash.ToLowerInvariant()
    ...
    $buildOutputs = @(Get-ChildItem `
        -LiteralPath (Join-Path $app "src-tauri\target\release") `
        -Filter "your-cloud-app.exe" -File)
    $installers = @(Get-ChildItem `
        -LiteralPath (Join-Path $app "src-tauri\target\release\bundle\msi") `
        -Filter "*.msi" -File)
    ...
    $appFileName = "your-cloud-app.exe"
    ...
    if ($null -eq $packagedExecutable -or $null -eq $packagedHelper -or
        ...
        throw "MSI must package the exact native helper beside the App"
    }
    ...
    $installedApp = $installedExecutables |
        Where-Object { $_.Name -eq $appFileName } |
        Select-Object -First 1
    $installedNativeHelper = $installedExecutables |
        Where-Object { $_.Name -eq $helperFileName } |
        Select-Object -First 1
    if ($null -eq $installedApp -or
        $null -eq $installedNativeHelper -or
        -not [string]::Equals(
            $shortcutTarget,
            $installedApp.FullName,
            [StringComparison]::OrdinalIgnoreCase
        ) -or
        -not [string]::Equals(
            $installedApp.DirectoryName,
            $installedNativeHelper.DirectoryName,
            [StringComparison]::OrdinalIgnoreCase
        )) {
        throw "shortcut target and installed native helper are not the exact App siblings"
    }
    ...
    Write-Host "CI Windows: installed helper refused a direct non-App parent"
    ...
    $applicationData = Join-Path $automationRoamingData "fr.your-cloud.app"
    ...
    Write-Host "CI Windows: installed App will run as a bounded standard user"
    ...
    if (-not [string]::Equals(
        ...
        throw "installed App did not start under the bounded standard-user account"
    }
    ...
    for ($attempt = 0; $attempt -lt 120; $attempt++) {
        ...
        if ($automationProcess.HasExited) {
            throw "installed App exited before publishing DevToolsActivePort"
        }
    ...
    for ($attempt = 0; $attempt -lt 120; $attempt++) {
        $automationProcess.Refresh()
        if ($automationProcess.HasExited) {
            throw "installed App exited before exposing its bounded WebView2 debugger"
        }
    ...
    try {
        Invoke-BoundedNative `
            ...
            -TimeoutSeconds 600 `
            -Arguments @(
                (Join-Path $root "tests\checks\app-windows-ui-proof.py"),
    ...
    if ($remainingAutomationProcesses.Count -ne 0) {
        throw "automated App process remained after WebDriver cleanup"
    }
    ...
    if (-not [string]::Equals(
        ...
        throw "normal App smoke test escaped the bounded standard-user account"
    }
    Start-Sleep -Seconds 10
    $process.Refresh()
    if ($process.HasExited) {
        throw "installed App exited during the Windows smoke test"
    }
    ...
    if ($listeners.Count -ne 0) {
        throw "installed App or one of its children opened a TCP listener"
    }
    ...
    Invoke-CleanupAction $cleanupFailures "App process cleanup" {
        if ($null -ne $process) {
            Stop-BoundedProcessTree -Process $process
        }
    }
    ...
    Invoke-CleanupAction $cleanupFailures "product and driver process absence" {
        ...
        if ($remainingTrackedProcesses.Count -ne 0) {
            throw "App, driver or ephemeral-user process remained active"
        }
    }
    ...
    Invoke-CleanupAction $cleanupFailures "application data containment" {
        if ($null -ne $applicationData) {
            ...
            [void](Resolve-BoundedChildPath `
                ...
                -Description "Windows App test data")
        }
    }
...
$proofReport | Add-Member -Force -NotePropertyName "verified_artifacts" -NotePropertyValue ([ordered]@{
    ...
    native_helper = [ordered]@{
        ...
        installed_matches_package = $true
        installed_as_exact_app_sibling = $true
        packaged_and_installed_without_reparse_point = $true
        authenticode_signer_matches_app_and_msi = $true
        pe_gate = $peGateProof
    }
})
...
Write-Host "PASS: Windows MSI administrative image exposes exactly the same-signed installable App and native helper executables"
Write-Host "PASS: packaged and installed helper hashes match, sibling paths are direct and non-reparse"
Write-Host "PASS: installed App launched and opened no TCP listener"
