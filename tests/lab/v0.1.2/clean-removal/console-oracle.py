#!/usr/bin/env python3
"""Mène la Console installée jusqu'au formulaire d'association, et pas plus loin.
...
Le pilote est le produit installé, `/usr/bin/your-cloud-console`, conduit par
`tauri-driver` devant le WebKitWebDriver avec lequel la Console Linux peint.
...
def reach_vault(driver: Driver, report: dict) -> None:
    """D'une Console froide au coffre créé, par le chemin du produit.

    Rien n'est semé : les deux secrets sont ceux que la Console a générés, relus
    sur l'écran qui les a affichés, et rendus à la Console qui les redemande.
    ...
    if not isinstance(secrets, list) or len(secrets) != 2:
        raise RuntimeError(f"la Console a rendu {secrets!r} plutôt que deux secrets")
    ...
    report["vault"] = "créé par la Console elle-même, sur la machine de cette passe"
...
def reach_association_form(driver: Driver, report: dict) -> None:
    ...
    usage unique — un autre périmètre. Ce qui est mesuré ici est que la Console
