...
def associate(driver: Driver, sheet: dict, recovery_code: str, report: dict) -> None:
    """Associate the real App with the real Controller, from the sheet.
    ...
    The global recovery code is the App's own, generated one screen earlier
    ...
    # heading: which view the App lands on afterwards is its decision, and
    ...
    report["association"] = {
        ...
        "reached": "the App crossed to its post-association views against a live Controller",
    }
...
def select_infrastructure(driver: Driver) -> None:
    """Enter the one infrastructure this App is associated with."""
...
def attach_machine(driver: Driver, machine_id: str, report: dict) -> None:
    ...
    `machine_not_active`. It is done from the App because that is where a
    human does it, and because the projection the App must read to sign —
    ...
    # response this pilot inspected: what the App shows is the claim.
    click_then_wait(
        driver,
        "Confirmer",
    ...
    report["attached"] = {
        ...
        "read_back": "the machine is listed by the App's own Parc view",
    }
...
def freeze_definition(driver: Driver, slug: str, repository: str, port: int, report: dict) -> None:
    """Write a definition in the App and freeze it.
    ...
    **no origin**, which is a configuration the product supports and enforces —
    ...
    # freeze that the App refused reddens here rather than letting the
    ...
    report["frozen"] = {"slug": slug, "by": "the App's own freeze gesture"}
...
def build_pair(driver: Driver, machine: str, digest: str, port: int, report: dict) -> None:
    ...
    report["pair"] = {
        ...
        "claim": "built and frozen by the Controller, rendered as sentences by the App",
    }
...
def main() -> int:
    ...
    try:
        ...
        if arguments.stage == "associate":
            ...
            # stages can open the same App. It is a secret of this run
        ...
        if arguments.stage == "touch":
            ...
            # the App to open a new, legitimate one so the interceptor has a
