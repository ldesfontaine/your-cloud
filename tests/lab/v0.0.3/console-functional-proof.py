#!/usr/bin/env python3
"""Drive secret-bearing Console flows without printing or capturing secrets."""
...
def pair(driver: Driver, secret_path: pathlib.Path, sheet_path: pathlib.Path) -> None:
    ...
    while time.monotonic() < deadline:
        ...
        if outcome["heading"] == "Synthèse":
            break
        if outcome["failure"]:
            raise AssertionError(f"pairing rejected by the Console: {outcome['failure']}")
        time.sleep(0.25)
    else:
...
def main() -> int:
    ...
    parser.add_argument("--application", default="/usr/bin/your-cloud-console")
    ...
    try:
        ...
        driver.close()
    print(f"console_functional={args.operation} result=pass secret_output=redacted")
    return 0
