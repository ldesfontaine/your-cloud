# Chaîne d'observation — la frontière, puis la carte

> **Ce document fixe la frontière de la chaîne d'observation** : ce que le
> Daemon relève, ce qu'il refuse, ce que le Relay ne peut pas lui demander, et
> les bornes de son tampon. Il la décrit ensuite — où le code s'exécute, qui
> appelle quoi, quelles données circulent. Ce qu'il fixe engage le produit :
> une capacité qui contredirait cette frontière est un défaut, pas une
> variante. Les rapports sous [`docs/lab/`](../lab/) portent seuls les dates,
...
Une [édition HTML autonome et visuelle](../html/chaine-observation.html) en
donne une vue dérivée.
