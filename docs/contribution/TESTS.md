...
est entièrement vert sur le candidat produit `f3fef79`. Il ajoute à la preuve
Linux le Job Object Windows, les branches hostiles de lancement et de récolte,
le helper empaqueté, le gate PE et le dispatch Tauri vivant. Le
[rapport Windows](../lab/v1-bootstrap-ipc-windows.md) conserve les résultats et
...
enregistre l'implémentation, ses tentatives diagnostiques et sa preuve
fonctionnelle. Des sous-cas Linux ont d'abord réussi sur des SHA intermédiaires, mais
...
cette étape intermédiaire, sans fermer #45. `c8643b0` ajoute ensuite
...
[`tests/lab/v0.1.0/app-reflow/prove`](../../tests/lab/v0.1.0/app-reflow/prove),
dont le rapport est
...
`.deb` hors ligne sur `lab-app`, l'installe, et conduit le processus du
...
`lab-app`, une autre VM LAB isolée ou un runner CI jetable explicitement
...
- **Poste de développement** : les contrôles statiques seulement —
  `tools/check-docs`, le contrat des sources App, `cargo fmt --check` et le
...
- [`tests/lab/v0.0.3/`](../../tests/lab/v0.0.3/) contient les pilotes bornés de
  la preuve Linux assistée ; `tests/checks/app-linux-ci` et
  `app-windows-ci.ps1` portent seulement les smokes natifs génériques ;
...
d'authentification App–Controller, le stockage et la projection déjà
...
La porte Linux emploie les six VM `v1-full` existantes : `lab-app` construit
puis exécute après retour à un snapshot propre ; `lab-app-recovery` porte la
seconde App hostile ; `lab-coordinateur` exécute Controller A et un
...
|---|---|---|---|
| sources et artefacts | `package.json` fournit la version App à Tauri, au SBOM et au manifeste candidat ; Cargo et le verrou npm doivent rester alignés ; même commit et même verrou frontend pour le `.deb` Linux et le `.msi` Windows ; manifeste, SHA-256, SBOM, provenance et signatures vérifiés | version divergente, identifiant d'exécution couplé à une version de livraison, artefact modifié, signature inconnue ou invalide, commit, cible, taille ou empreinte contradictoire | garde source et Linux exact prouvés ; lot Linux/Windows final accepté dans `30710037004` sur `3b8f81f` et lié par `#9` |
| enveloppe Tauri | frontend React, TypeScript et Vite embarqué ; opérations natives nommées ; aucun listener sur l'appareil App ou chargement de code distant | navigation distante, ressource active externe, appel réseau frontend, accès fichier ou shell non autorisé | Linux LAB prouvé ; installation, lancement, absence de listener et smoke WebView2 Windows exécutés dans `30710037004` sur `3b8f81f` |
| origine App–Controller | TLS 1.3 sur l'origine exacte avec certificat serveur, identité d'appareil et session humaine attendus | HTTP, mauvais nom, CA, port, query, fragment, redirection, proxy, certificat inconnu, révoqué ou d'un autre Controller | planifié, non exécuté |
...
| cardinalité `v0.0.3` | exactement un humain et un appareil App actifs par Controller, plusieurs Controllers restant isolés dans la même App | second appairage actif sans récupération, deux certificats métier actifs dans un Controller ou partage d'une clé/session entre Controllers | planifié, non exécuté |
...
puis lance l'App réelle et produit son smoke WebView2. Le runner Linux
...
deux imports Win32. Deux tentatives de contrôle croisé de toute l'App ont
...
et la [roadmap](../objectifs/v1/ROADMAP.md). Le gate ELF qui choisit un helper
séparé, sa fondation fail-closed, son lancement parent et le premier
consentement GTK3 sans secret ont reçu une preuve Linux le 2 août 2026,
conservée dans le
[rapport LAB dédié](../lab/v1-bootstrap-ipc-linux.md). Le bornage IPC #43 a
ensuite reçu son équivalent Windows, ses branches hostiles et son dispatch Tauri
vivant dans le [rapport natif exact](../lab/v1-bootstrap-ipc-windows.md). Les
autres scénarios restent planifiés ; ces passages ne prouvent ni une saisie
secrète, ni une connexion SSH, ni un succès métier d'amorçage.

Le workflow natif reste volontairement déclenché par `workflow_dispatch`, selon
le [contrat CI](CI.md). Le run `30753216798` a exécuté tout le workspace, les
deux scénarios Xvfb, les gates de paquet et les contrats Windows sur le candidat
exact `f3fef79`. Cette preuve hébergée reste distincte du LAB Linux historique
et ne crée aucune topologie métier.
...
#51 est exécutée : son
[rapport LAB](../lab/v0.1.0-personal-access-bounds.md) fixe
`MAX_BCRYPT_ROUNDS = 2048` depuis une calibration `bcrypt_pbkdf` d'environ
4,6 ms par round sur `lab-app`, et valide le décideur `sudo` contre cinq
captures réelles de Debian 13. L'exécution a corrigé une hypothèse : les
entrées Defaults réelles sont réparties sur plusieurs lignes, ce qu'un listing
tiré de la page de manuel n'exerce pas. Ces captures sont figées comme
fixtures. #51 ne prouve ni SSH, ni signature d'agent, ni envoi de mot de passe.

#52 possède désormais son périmètre LAB versionné plutôt que des scripts de
session : [`tests/lab/v0.1.0/personal-access/prove`](../../tests/lab/v0.1.0/personal-access/prove)
monte les deux côtés du périmètre — cinq comptes synthétiques et leurs commandes
forcées, trois `sshd` hors listes positives, les noms du résolveur, un
...
isolé avec `--ignored`, pour les cinq cas dont l'affichage est l'objet : un
helper lancé par le superviseur de l'App elle-même atteint réellement
l'agent personnel et ouvre sa fenêtre, une fenêtre d'élévation lancée par le
même superviseur ne reçoit jamais l'endpoint de cet agent, et les trois
propriétés de la fenêtre vivante — coupure par le chien de garde avant le repli
forcé, aucun descripteur hérité hors stdio, refus de toute mutation ultérieure
du périmètre — sont observées sur le helper lancé directement. Le rejeu du
3 août 2026 rend verts vingt-deux tests sans affichage et cinq sous `Xvfb` ; les
vingt-deux comprennent les cinq tests unitaires du superviseur de l'App,
que l'inclusion de sa source dans la suite amène avec elle.

Ce serveur X virtuel est tenu ouvert pour toute la passe (`-noreset`). Sans
cela il se régénère chaque fois que son dernier client se déconnecte, ce qui
arrive des dizaines de fois par passe puisque les seuls clients sont les helpers
observés et les sondes `xdotool` qui les observent ; une connexion acceptée
pendant cette régénération est fermée avant sa réponse d'ouverture, et le helper
annonce alors honnêtement un affichage qu'il n'a pas pu joindre. Le refus ne
visait pas le produit : sur la même session, un `xdpyinfo` de série est refusé
environ trois fois sur cent, et jamais quand la génération est tenue.

#37 possède son périmètre LAB versionné :
[`tests/lab/v0.1.0/signed-approval/prove`](../../tests/lab/v0.1.0/signed-approval/prove)
monte l'ancre d'approbation `root` et le répertoire anti-rejeu sur `lab-machine-1`,
exécute la matrice complète des acceptations et des refus, **arrête puis
redémarre réellement la machine** par le contrôleur du LAB, rejoue les refus,
puis démonte et prouve l'absence. Chaque refus est vérifié par sa raison et non
par un simple statut non nul, et chacun est encadré par le contrôle positif qui
isole sa seule différence. Le signataire employé par le harnais est une fixture
synthétique qui réécrit la même transcription canonique ; l'interopérabilité
réelle avec le cœur natif est prouvée séparément par le vecteur déterministe
épinglé, où la signature produite par Rust est vérifiée par Go. Le passage porte
sur l'arbre de travail et non sur un candidat publié : il documente le harnais
et les propriétés, il ne ferme pas #13.

#39 possède son périmètre LAB versionné :
[`tests/lab/v0.1.0/machine-identity/prove`](../../tests/lab/v0.1.0/machine-identity/prove)
enrôle **deux** machines par l'accès personnel — le minimum, puisque la propriété
du palier est que l'identité de l'une est refusée sur l'autre — relit sur chaque
machine ce qu'elle détient vraiment, fait juger chaque relevé par le module
compilé, éprouve un vrai `sshd` et un vrai `sudo` capacité par capacité à côté de
leurs contrôles positifs, fait parcourir au Controller le chemin qu'il vient
d'installer, puis n'active que les rôles approuvés. La frontière que la première
passe avait laissée ouverte est levée dans la même passe : `placement::propose`
place désormais le rôle Agent avec **ses** exigences — un Agent tourne sur toute
machine gérée, donc aucun critère de confidentialité de placement ne le concerne,
et son plancher de ressources est dérivé de sa propre unité et non emprunté au
Controller. `RoleOutsideThisPalier` ne nomme plus que l'Auxiliaire, qui n'est pas
un rôle qu'on place mais un mode ponctuel du même artefact. Ce que la passe ne
prouve pas : aucune unité n'est réellement démarrée, et le plancher de
l'Agent est décidé, pas mesuré.
...
|---|---|---|---|
| lot App et serveur | installateur contenant l'Assistant, l'unique `.deb` Debian 13 `amd64`, ses définitions statiques et le manifeste signé exact ; signature, cible, version, taille, SHA-256 et dépendances hors ligne vérifiés avant privilège ; `/usr/lib/your-cloud/your-cloud` `root:root` `0755` sans setuid, setgid ni capacité et exactement trois unités `root:root` `0644` inactives sous `/usr/lib/systemd/system` ; état propre à la machine géré par l'Assistant ; binaire installé et vérifié avant la clé forcée ; retour à l'absence ou à la version antérieure avant transfert d'autorité ; entrée Auxiliaire initiale en lecture seule et en refus de mutation | paquet ou manifeste altéré, cible, version, taille ou empreinte divergente, dépendance exigeant le réseau, fichier ou unité supplémentaire, propriétaire ou mode divergent, setuid, setgid, capacité ou unité activée par l'installation, script mainteneur interactif ou activant un rôle, secret ou configuration propre à une machine dans le paquet, état à demi configuré présenté comme sain, retrait aveugle après coupure, binaire privilégié téléchargé à la volée, clé activée avant le binaire, mutation acceptée avant son contrat, `arm64` accepté sans preuve ; certificat Windows synthétique présenté comme signature publique | helper Linux construit et contrôlé puis `.deb` App installé pour #43 ; sous Windows, le `.msi` installé contient exactement les deux exécutables installables et le certificat synthétique prouve seulement la mécanique Authenticode. #38 exécutée le 5 août 2026 depuis l'arbre de travail : [rapport LAB](../lab/v0.1.0-controller-install.md) et harnais versionné [`tests/lab/v0.1.0/controller-install`](../../tests/lab/v0.1.0/controller-install/). Le paquet serveur existe : `dpkg -L` n'inventorie que le binaire `root:root` `0755` et les trois unités `root:root` `0644` livrées inactives, sans setuid, setgid ni capacité, et une seule unité est activée. Le manifeste signé lie version, cible, taille et SHA-256, et huit refus tirent chacun leur propre raison avant tout privilège. Le Controller tourne sous compte dynamique sans capacité, budgets `TasksMax=128` et `MemoryMax=384 Mio`, secrets `root:root` `0600` remis par systemd seul ; il garde le même PID après l'arrêt réel de `lab-app`. Quatre points d'arrêt rendent un registre que le déroulé reconstruit, machine rendue à son état initial à chaque fois. Trois épreuves par mutation font rougir la suite. L'embarquement du lot dans l'installateur de l'App et le scellement de l'ancre restent planifiés |
| helper natif et cycle de vie | binaire compagnon `your-cloud-native-bootstrap-assistant` lancé une fois par consentement avec la garde `--native-bootstrap-assistant`, graphe autonome sans Tauri, Wry, Tao, WebKit ou JavaScriptCore ; pipes anonymes typés portant seulement le périmètre public immuable et des états expurgés ; expiration native monotone fixe de 300 secondes ; fermeture des enfants après succès, refus, annulation, timeout, EOF, mort du parent et crash | durée fournie ou prolongée par le frontend, dépendance directe, transitive ou chargée dynamiquement vers WebKit/JavaScriptCore, seconde WebView, secret dans l'IPC frontend, arguments, environnement, URL, descripteur hérité inattendu, fichier temporaire, journal ou dump ; helper ou enfant survivant, état réutilisé entre deux opérations | bornage #43 prouvé sous Linux et Windows : gates Cargo/ELF, imports PE normaux et différés, framing, échéance native, lancement App, WebView Tauri vivante, session unique, forge et rejeu refusés, groupe Linux, Job Windows avec racine et descendant, branches avant reprise, récolte autonome et refus de relance si le nettoyage n'est pas prouvable. Le gate PE ne prouve pas l'absence universelle de chargement dynamique. #45 prouve dans `30779157351` sur `c0569d0` le pair/parent lié, le périmètre immuable, l'annulation coopérative, la collecte secrète et les protections mémoire ; l'issue est fermée. Les vrais descendants SSH et privilégiés restent à #42 |
| moteur SSH et agent personnel | `russh 0.62.4` épinglé, algorithmes sur liste positive, clé d'hôte exacte ; socket Unix absolu appartenant à l'utilisateur courant sous Linux ou pipe `\\.\pipe\openssh-ssh-agent` sous Windows ; une clé et un budget fini de signatures pour l'authentification exacte | clé d'hôte inattendue, DSA, DES, compression ou `ssh-rsa` SHA-1, autre endpoint d'agent, deuxième signature, message de signature libre, TOFU, écriture de `known_hosts`, shell, PTY, SFTP, X11, redirection, transfert d'agent ou commande générale | listes positives de clé d'hôte et d'identité implémentées et prouvées par #51, avec refus de DSA, ECDSA, FIDO, certificats, `ssh-rsa` SHA-1 et de toute compression ; la connexion réelle, l'agent et le budget de signatures sont exercés par la suite `personal-access-contract` et son harnais LAB [`tests/lab/v0.1.0/personal-access`](../../tests/lab/v0.1.0/personal-access/), verte le 3 août 2026 depuis l'arbre de travail et non depuis un candidat publié : vingt-deux tests sans affichage et cinq sous le serveur X virtuel du harnais, ces cinq-là rejoués douze fois de suite sans un échec. Ils ont bien été instables — sept échecs sur onze exécutions, un helper sortant indisponible au lieu d'ouvrir sa fenêtre — jusqu'à ce que la cause soit établie : le serveur X du harnais se régénérait dès que son dernier client se déconnectait et fermait sans réponse la connexion arrivée pendant cette régénération, refus qui frappait n'importe quel client X et non le produit. Le harnais tient désormais la génération pour toute la passe et aucune source du produit n'a bougé ; #52 reste ouverte et #42 avec elle |
| repli par fichier de clé | sélecteur natif ouvrant sans réécriture une clé `OPENSSH PRIVATE KEY` chiffrée bcrypt + `aes256-ctr`, Ed25519 ou RSA d'au moins 3072 bits ; octets, passphrase et clé déchiffrée zéroïsés sur sortie contrôlée | clé en clair, RSA trop courte, PKCS#1, PKCS#8, SEC1, PPK, autre KDF ou chiffrement, fichier trop grand ou remplacé pendant l'ouverture, passphrase ou clé retrouvée dans un état persistant ou un artefact | dialogue secret implémenté et prouvé par #45 ; validation d'enveloppe avant KDF, borne de rounds calibrée et refus des formats étrangers implémentés et prouvés par #51 ; ouverture réelle de la clé, substitution du fichier pendant la lecture et zéroïsation restent à #53 avant fermeture de #42 |
| déclaration et audit | endpoints fournis un par un, clé d'hôte confirmée, audit SSH strictement en lecture seule, Debian 13 `amd64`, rôles et ressources rendus ; chaque endpoint et sa clé d'hôte revérifiés depuis le Controller avant mutation des cibles | scan du LAN, plage ou fournisseur, clé d'hôte acceptée silencieusement, mutation pendant l'audit, cible ou rôle incompatible proposé, cible joignable depuis le laptop mais pas depuis le Controller | #36 exécutée le 6 août 2026 depuis l'arbre de travail : [rapport LAB](../lab/v0.1.0-endpoint-audit.md), suite portée par le harnais [`tests/lab/v0.1.0/personal-access`](../../tests/lab/v0.1.0/personal-access/). L'audit n'écrit rien, et c'est prouvé plutôt qu'affirmé : l'empreinte de la machine — chemins, tailles, dates de modification et modes — est identique avant et après sept audits, et l'endpoint canari ne gagne aucune connexion, son contrôle positif appartenant au cas. Chaque incompatibilité est refusée par la déviation qu'elle porte et par elle seule. Les machines déviantes sont **synthétiques** : ce sont des comptes dont les commandes forcées répondent ce qu'une telle machine répondrait, et aucune vraie Ubuntu ni `aarch64` n'existe dans ce LAB. La confirmation humaine de la clé d'hôte reste à #45, l'installation à #38 |
| consentement privilégié | dialogue GTK3 direct sous Linux et Win32 modal `EDIT | ES_PASSWORD` sous Windows, répétant empreintes, étape, actions et expiration ; `sudo -n` tenté d'abord, puis un unique envoi sans PTY à `sudo -k -S` pour le chemin absolu autorisé ; accès `root` explicite | secret livré par IPC, cible, action ou expiration substituée, essai `root` implicite, réutilisation ou retry, prompt distant inattendu, shell ou interpolation, politique distante non bornable ou journalisation d'entrée susceptible de capturer le mot de passe | #45 prouve dans `30770893733` les dialogues GTK3/Win32, la saisie masquée, les refus de mutation et l'événement terminal public `Unavailable`. `sudo`, l'accès `root` et leur politique distante appartiennent à #51/#54 puis #42 |
| effacement et anti-dump | buffers du helper zéroïsés ; sous Linux mort avec parent, `PR_SET_DUMPABLE=0`, `RLIMIT_CORE=0`, `mlock` et `MADV_DONTDUMP` disponibles ; sous Windows Job Object, `VirtualLock` et enregistrement Windows Error Reporting en défense en profondeur ; `LocalDumps` administrateur explicitement hors garantie et limites publiées | faux résultat « aucun secret possible » après crash, core dump ou fichier d'échange sans inspection ; descendant survivant ; copie propre au produit retrouvée après sortie contrôlée ; `WerRegisterExcludedMemoryBlock` présenté comme garantie face à un admin ; dump, répertoire ou inscription de registre de la fixture non supprimé | Job Object, handle sentinelle, racine, vrai descendant, terminaison et récolte prouvés pour #43. #45 prouve `ProtectedSecret` 4096, wipe, `mmap`/`mlock`/`MADV_DONTDUMP`, contrôle `gcore` présent et canari absent, `VirtualAlloc`/`VirtualLock` et enregistrement WER best effort. Les runs rouges `30768351689` et `30768749538` caractérisent le dump administrateur avec contrôle et canari présents. `30769440106` reste intermédiaire ; `c8643b0` ajoute `remove_and_prove_absent`, validé avec les absences de registre dans `30770893733`. Les vrais descendants SSH et privilégiés restent à #42 |
| placement et autonomie | Controller privé normalement allumé, cohabitation isolée sur petite infrastructure et recommandation dédiée lorsque taille ou risque augmentent ; fermeture de l'App sans arrêt du contrôle ou des services ; perte du Controller sans arrêt des services des autres hôtes et interruption colocalisée rendue visible | Controller permanent sur le laptop ou proposé par défaut sur le VPS public ; partage de compte, secret, répertoire ou budget entre rôles ; promesse de continuité d'un service placé sur l'hôte perdu | planifié, non exécuté |
| identités SSH par machine | paire différente sur chaque machine, clé privée root-owned fournie au seul Controller par credentials systemd, clé publique vérifiée avant transfert d'autorité | même clé sur deux machines, clé d'une machine acceptée par une autre, clé privée visible à l'App ou à l'Agent, clé personnelle retirée | #39 exécutée le 5 août 2026 depuis l'arbre de travail : [rapport LAB](../lab/v0.1.0-machine-identity.md) et harnais versionné [`tests/lab/v0.1.0/machine-identity`](../../tests/lab/v0.1.0/machine-identity/). Deux machines sont enrôlées — le minimum, puisque la propriété est que l'identité de l'une est refusée sur l'autre. Un vrai `sshd` répond `Permission denied (publickey)` dans les deux sens croisés, et la porte compilée rend `ForeignIdentity`, `Unattributed` et `SharedIdentity` sous leur propre nom. Le Controller parcourt lui-même le nouveau chemin avant toute activation, et seuls les rôles explicitement approuvés sont activés : **le rôle Agent est ouvert par #36 dans cette même passe**, avec ses exigences propres, et son activation est prouvée avec son contrôle négatif et le refus d'un chemin non vérifié. Six points d'arrêt rendent un registre que le déroulé reconstruit ; une coupure rend `INCOMPLETE` et nomme ce qui reste. Deux épreuves par mutation font rougir la suite. Aucune unité n'est réellement démarrée et le plancher de ressources de l'Agent est décidé, pas mesuré |
| commande forcée Auxiliaire | compte technique verrouillé sans mot de passe ; fichier de clés, parents et chemin absolu du binaire root-owned et non inscriptibles ; restrictions SSH et élévation exacte avec environnement réinitialisé ; plan typé sur stdin | option SSH retirée, répertoire remplaçable, binaire ou parent inscriptible, shell, PTY, SFTP, rc, X11, transfert, `environment=`, `SETENV`, règle `sudo` générale, sous-commande, argument, chemin ou opération libre | #39 exécutée le 5 août 2026 depuis l'arbre de travail : [rapport LAB](../lab/v0.1.0-machine-identity.md). Le compte `your-cloud-auxiliary` est verrouillé sans mot de passe, sans groupe supplémentaire et sans répertoire personnel ; les chaînes `stat` du fichier de clés et du binaire sont jugées par le module compilé. `sshd -T` résout `no` sur PTY, X11, `permituserrc`, transfert de port et d'agent, tunnel, environnement utilisateur et mot de passe, à côté d'un compte témoin qui résout `yes` au même instant. Chaque capacité est refusée vivante : commande du client jamais exécutée, PTY refusé, SFTP refusé, fichier rc jamais lu, transferts sans un octet. La règle `sudo` n'autorise que l'invocation exacte `/usr/lib/your-cloud/your-cloud auxiliary approve`, sans `SETENV` ; un argument libre, un shell et une variable d'environnement sont refusés chacun par sa propre raison. La sous-commande retenue est `auxiliary` et `aux` a disparu de la documentation |
| approbation et anti-rejeu | cœur natif signant l'enveloppe canonique plan + rollback ; clé publique, infrastructure, machine, époque et successeur exact de la séquence vérifiés localement ; séquence consommée avant mutation et persistante après redémarrage | signature libre accessible au frontend, plan forgé ou altéré par un Controller compromis, mauvaise clé ou époque, séquence sautée, ancienne, concurrente ou rejouée avant et après restart | #37 exécutée le 5 août 2026 depuis l'arbre de travail : [rapport LAB](../lab/v0.1.0-signed-approval.md) et harnais versionné [`tests/lab/v0.1.0/signed-approval`](../../tests/lab/v0.1.0/signed-approval/). L'enveloppe versionnée lie les douze champs, un vecteur déterministe unique est épinglé des deux côtés et la signature produite par le code Rust est vérifiée par le code Go. `lab-machine-1` est réellement arrêtée puis redémarrée : la position anti-rejeu est retrouvée octet pour octet et rejeu, séquence ancienne, sautée et concurrente restent refusés. Trois épreuves par mutation font rougir la suite. Aucune mutation n'est exécutée et aucune commande Tauri n'atteint le module de signature ; l'installation de la commande forcée SSH a été prouvée par #39 le même jour, et son lancement depuis le Controller appartient à la `v0.1.2` (#126) — l'attribution à #54 était une erreur, corrigée le 9 août 2026 par [`TRAJET-DE-COMMANDE.md`](../architecture/TRAJET-DE-COMMANDE.md) |
| récupération de App et actions | rotation du certificat avec même clé humaine conservant l'ancre ; nouvelle clé humaine verrouillant les actions jusqu'à rotation cible par cible via l'Assistant et l'accès personnel | Controller tournant seul l'ancre, code de récupération signant une action, ancienne et nouvelle clés humaines acceptées ensemble, action disponible pendant une rotation partielle | planifié, non exécuté |
| remplacement du Controller | choix explicite, nouvelle association App, Agents compatibles réutilisés, lecteur Relay limité au nouveau Controller, nouvelle époque et toutes identités exposées tournées avant retrait des seules anciennes identités marquées | remplacement automatique sur panne ambiguë, Controller compromis non isolé ou hôte non assaini, ancien lecteur/session/clé encore actif, Controller sain écrasé, clé personnelle ou inconnue retirée, ancienne identité retirée avant preuve de la nouvelle | #40 exécutée le 5 août 2026 depuis l'arbre de travail : [rapport LAB](../lab/v0.1.0-controller-replacement.md) et harnais versionné [`tests/lab/v0.1.0/controller-replacement`](../../tests/lab/v0.1.0/controller-replacement/). `lab-app` est **réellement arrêtée** : la panne jugée est une vraie panne, observée depuis deux postes indépendants d'espèces différentes — une tentative TCP depuis `lab-machine-1` et l'état du domaine rapporté par l'hyperviseur — et le silence de 310 s qui qualifie la perte est un silence qui s'est écoulé. Un Controller qui répond encore rend `ControllerStillAnswering`, une panne que personne n'a demandé de traiter rend `NotRequestedByTheUser`, un silence plus jeune que la borne rend `Ambiguous(SilenceTooYoung)`. La perte matérielle et la compromission sont **deux séquences** que la porte rend elle-même, la seconde commençant par `isolate` et exigeant `IsolationNotVerified` sans isolement. Le socket du lecteur est relevé fermé à chacune des quatre transitions et un manifeste nommant deux Controllers rend `TwoControllers`. L'ancienne identité n'est retirée qu'après que la nouvelle a répondu sur la même machine — le retrait exige le témoin `PathVerified` de #39 par signature — puis un vrai `sshd` répond `Permission denied (publickey)` à l'ancienne sur les deux machines. La clé personnelle d'un vrai compte est nommée parmi les conservées. Les quatre états sont reconstruits depuis la machine, le désaccord entre le fichier et le fil rendant `unknown`, et une coupure avant transfert est défaite par le registre de #38. Trois épreuves par mutation font rougir la suite. Aucun Controller Go ne tourne, aucun vrai lecteur Relay n'est servi, l'isolement n'est pas exécuté et il n'y a pas de VM hostile distincte |
...
suivant `30769440106` réussit entièrement, mais reste intermédiaire. Le run
`30770893733` réussit ensuite la matrice entière sur `b76ded8`, qui inclut
`c8643b0`, et remplace cet état par une preuve fonctionnelle acquise.
...
prouver le répertoire absent avant verdict. `30770893733` valide cette séquence
sur `b76ded8` dans le même test que les absences `LocalDumps` et `AeDebug`.
...
Le run fonctionnel `30770893733`, entièrement vert du
`2026-08-02T22:48:02Z` au `2026-08-02T23:14:31Z`, relie contrôles génériques
`91557743005`, Plumber `91557743018`, Windows `91557829499` et Linux
`91557829507` au SHA `b76ded8`. Il publie Windows `8840757379`
(`sha256:8ee952ba6a265e4ad94289eb265cd19ab4ab5bef472f4d4d2dcbb6d56c38b973`,
JSON `a2efa0a548d131e1000b4f03b7a7a6a511f45f2ec42e39640f2148dc32a8dfe0`),
Linux `8840658837`
(`sha256:56e904626e35c06fefe4966850b307f2dbc97eb075b83831f5fa15845e4cf58f`,
JSON `ab150f2527a35633196c2e4cef15d2cb90fb8ef78c2a366dd4cbb7873f3e3303`)
et Plumber `8840483746`
(`sha256:98499038c307b2d3dddeca46fc4195dbbf51b50a27cbe1a33b03609cfef95763`,
JSON `629dc3b0e73b192b92b9b89fe5d4bd9cce6ce18041cd3c8283a27511ed0c5ff2`).
L'inventaire exact rend 20 PNG et 3 JSON, tous réguliers et non vides. Les
...
| syntaxe du générateur Python de restitution | automatique | automatique | automatique | automatique | compilation seule de `tests/lab/v0.0.1/report/renderer.py` ; le rendu réel reste vérifié dans `lab-app` |
| résultat structuré Plumber absent, ambigu, incomplet, sauté ou dégradé | automatique | automatique | automatique | automatique | 23 cas de frontière (20 refus, 3 acceptations), liaison au lot et refus intégré d'un tag mutable exécutés dans `lab-app` ; porte finale verte dans `30710037004` sur `3b8f81f`, liée par `#9` |
...
| `go test -count=1 ./...` | automatique | automatique | automatique | automatique | mode `lab` sur `lab-app` root ou mode `ci` sur runner distant non privilégié |
...
Le Markdown et la page HTML sont générés dans `lab-app` depuis les seuls
...
| les deux premières analyses Plumber n'ont pas trouvé de dépôt Git dans `lab-app` | l'image LAB ne contient pas `git`, alors que Plumber lit trois métadonnées avant les workflows | doublure bornée à l'origin public, la racine temporaire et l'identifiant dérivé du lot ; toute autre commande sort avec `2` | le runner GitHub utilisera le vrai checkout ; la doublure reste limitée à la simulation LAB |
...
| le premier déploiement d'un service utilisateur échouait sur `statfs …/volumes/srv: permission denied` (`#121`) | la placement ne nommait que la racine durable et le chemin hôte de chaque volume ; le répertoire intermédiaire d'un chemin conteneur à plusieurs segments était créé par `MkdirAll` en chemin, donc laissé `root:root 0700`, et le moteur rootless ne pouvait pas traverser jusqu'à sa propre monture | `placement.durableDirectories` nomme désormais **chaque** répertoire entre la racine durable et chaque volume, parents d'abord et sans doublon ; la couture crée, `chmod` et `chown` exactement ce qu'on lui nomme | `TestEveryDirectoryOnTheWayToAVolumeIsNamed` fixe la liste attendue, l'ordre parents d'abord et l'unicité, et vérifie dans le même test que le profil privé livré ne nomme toujours qu'un répertoire |
| une valeur approuvée contenant une espace atteignait le conteneur tronquée à sa première espace, sans erreur (`#121`) | la fiche écrivait `Environment=CLÉ=valeur` verbatim ; le générateur qui la lit termine une valeur non citée à la première espace et développe `%` comme un spécificateur — un humain approuvait `Your Cloud notes` et le conteneur recevait `Your` | `renderSheet` passe chaque ligne par `environmentLine` : `%` toujours doublé, et une ligne portant une espace, un guillemet ou une contre-oblique écrite entre guillemets avec ses échappements — une ligne qui n'en porte aucun reste verbatim, donc les fiches des profils livrés ne bougent pas d'un octet | `TestAnApprovedValueReachesTheContainerWhole` tient la propriété plutôt que l'orthographe : la relecture du lecteur, appliquée à ce qui est écrit, rend la valeur approuvée ; la fiche témoin de la troisième porte garde la ligne citée et les deux non citées |
...
2. `tests/checks/source-v0.0.1` est l'entrée source commune. Le mode `lab`
   prépare le lot dans `lab-app` ; le mode `ci` utilise un runner distant
...
3. Cette entrée refuse un mode implicite, un mode `lab` qui n'est pas root sur
   `lab-app`, un mode `ci` root ou hors GitHub Actions, un lien
...
2. La page est servie seulement dans `lab-app` sous `nobody` sur loopback ;
