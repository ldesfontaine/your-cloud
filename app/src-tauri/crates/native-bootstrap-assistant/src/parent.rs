...
const APP_EXECUTABLE_NAME: &str = "your-cloud-app.exe";
const HELPER_EXECUTABLE_NAME: &str = "your-cloud-native-bootstrap-assistant.exe";
const LINUX_APP_PATH: &str = "/usr/bin/your-cloud-app";
...
fn exact_linux_layout(parent: &Path, helper: &Path, directory: &Path) -> bool {
    parent == Path::new(LINUX_APP_PATH)
...
fn exact_windows_layout(parent: &Path, helper: &Path) -> bool {
    helper.file_name() == Some(std::ffi::OsStr::new(HELPER_EXECUTABLE_NAME))
        && helper
            .parent()
            .is_some_and(|directory| directory.join(APP_EXECUTABLE_NAME) == parent)
}
...
mod linux {
    ...
    use super::{
        exact_linux_layout, secure_unix_node, UnbufferedStandardInput, LINUX_APP_PATH,
    ...
    pub(super) fn verify(input: &UnbufferedStandardInput) -> Result<Guard, ()> {
        let expected_parent = Path::new(LINUX_APP_PATH);
...
mod windows {
    ...
    pub(super) fn verify(input: &UnbufferedStandardInput) -> Result<Guard, ()> {
        ...
        let expected_parent_path = installation_directory.join(super::APP_EXECUTABLE_NAME);
...
mod tests {
    ...
    fn windows_policy_requires_exact_siblings() {
        ...
        let parent = Path::new("/Program Files/Your Cloud/your-cloud-app.exe");
        assert!(exact_windows_layout(parent, helper));
        assert!(!exact_windows_layout(
            Path::new("/Elsewhere/your-cloud-app.exe"),
            helper,
        ));
        assert!(!exact_windows_layout(
            parent,
