...
pub const fn channels_for(step: Step) -> usize {
    match step {
        ...
        Step::AssociateApp | Step::Preflight => 0,
    }
}
...
pub fn channel_budget(action: your_cloud_bootstrap_protocol::BootstrapAction) -> usize {
    ...
    if opened == 0 {
        ...
        // jamais depuis un champ qu'une App affirmerait. Les canaux de
...
mod tests {
    ...
    fn the_steps_without_an_act_answer_with_an_empty_list() {
        for step in [Step::TransferBundle, Step::AssociateApp, Step::Preflight] {
            assert!(
