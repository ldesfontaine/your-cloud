...
mod canary_scan;
/// The App's own supervisor, compiled from the App's own source the
...
/// Lease the App gives a supervised helper here. It is generous on
...
/// The scope the App submits for a real personal access against the LAB.
///
...
fn await_supervised_window(
    ...
    loop {
        ...
        assert!(
            Instant::now() < deadline,
            "no visible window owned by a child of this App appeared"
        );
        thread::sleep(POLL_INTERVAL);
    }
}
...
/// A helper the *App* launched really reaches the personal agent.
///
...
/// the personal access title, owned by a child of this App, is a proof
...
fn a_app_launched_helper_reaches_the_personal_agent_and_opens_its_window() {
    ...
    supervisor
        ...
        .expect("the App supervisor must launch the helper");
    ...
    assert!(
        ...
        "the helper reached its window without the endpoint the App holds: {environment:?}"
    );

    // Cancelling from the App closes the window, the helper and the agent
    // connection it opened, exactly as any other terminal path would.
    supervisor
        .cancel(SUPERVISED_REQUEST_ID)
        .expect("the App must be able to cancel its own helper");
    let deadline = Instant::now() + SETTLE_TIMEOUT;
...
/// not be able to reach one even though the App that launched it can. The
...
// socket pair, exactly as the process contract does: the App's supervisor
...
/// Frames a scope the way the App's transport does.
///
...
/// The shipped helper, invoked exactly as the App invokes it.
///
...
// session the elevation above uses. Every case below drives what the App
