...
// The one request of this App whose bound is not the common four kilobytes,
...
// admits from being one this App could never send.
...
pub(crate) enum NetworkError {
    ...
    /// The Controller answered and this App refused its answer.
    ///
    ...
    /// answer to write into this App's own account of itself.
    ...
    ///
    /// The string is one of the closed list this App holds in
    ...
    /// letting a remote name a public code: a Controller can steer this App
...
impl NetworkError {
    ...
    pub(crate) fn public_code(&self) -> &'static str {
        match self {
            ...
            // closed list, so it belongs to this App's vocabulary exactly
...
impl NetworkState {
    ...
    /// a declaration must not move the revision an App caches its machines
    ...
    /// is written by this App from the context of this route, because a
    ...
    /// App caches its machines against.
    ///
    ...
    /// digest, and this App holds the two against one another with the very
    ...
    ///
    /// The App assembles no plan. It names a machine, a frozen revision and
    ...
    /// lives where the records are kept, and an App that filtered on the way
    /// in would be an App deciding what a human may know about what was
    ...
    ///
    /// This is the one request of this App whose effect leaves the
    ...
    /// and believes neither this App nor its own earlier freeze.
    ///
    /// The definition travels exactly when the operation's door pins one. It is
    /// the revision this App read back from the Controller, not one a
    ...
    /// business route of this App uses.
    ///
    /// The digest is not sent as an authority. It is this App's own answer,
        ...
        F: FnOnce(AssociationRecord) -> Result<(), NetworkError>,
    {
        ...
        association.device_private_key_pem = association
            .pending_device_private_key_pem
            .take()
            .ok_or(NetworkError::AppUnavailable)?;
        association.device_certificate_pem = association
            .pending_device_certificate_pem
            .take()
            .ok_or(NetworkError::AppUnavailable)?;
...
pub(crate) struct MachineView {
    ...
    /// is certain. It is what the App needs in order to sign the **exact
    /// successor** of the machine's sequence, and it is read rather than
    /// guessed: this App is trusted for neither the position nor the epoch,
...
/// The declared inventory as this App reads it.
///
...
/// this App announces them from the context of the route. A Controller that
...
/// The bounded history, oldest first, every machine together: the App
...
/// One dispatch as this App reads it back.
#[derive(Clone, Debug, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
...
/// What the App may choose about one deployment of a frozen revision, and
/// nothing else.
///
...
/// Everything an App may choose about a freeze, which is the document and its
...
/// of this App keeps the four kilobytes it has, because one named document
...
fn pairing_error(error: NetworkError) -> NetworkError {
    match error {
        ...
        | NetworkError::AppUnavailable => error,
...
/// not held, the state disagreed — things an App reports as a liaison
...
///
/// The returned string is this App's own literal, matched from what was
...
/// projection this App cannot read entirely is a projection it refuses
/// entirely, because an App that dropped the one line it could not parse
...
/// listing this App cannot verify entirely is a listing it refuses entirely,
...
///
/// A state or a reason this App does not know is a refusal and never a
...
mod tests {
    ...
    /// Présente à l'App un document de refus **tel que le Controller
    /// l'écrit** : mêmes en-têtes, même corps, même ordre de champs que
    /// `writeProblem`. Rien n'est simulé du côté de l'App — c'est son
    ...
    /// Le défaut que ce cas ferme : l'App ne connaissait aucun des six
    ...
    /// l'App un refus tel que le Controller l'émet : celle-ci le fait.
    #[test]
    ...
    /// et l'App ne doit pas le prendre pour lui.
    #[test]
    ...
    fn lab_app_enrollment_session_and_inventory() {
        ...
        let mut core = AppCore::new(directory.path().join("state"));
        ...
        let mut association = network
            .pair(input, 0, &generation, |candidate| {
                core.store_association(candidate, false)
                    .map(|_| ())
                    .map_err(|_| NetworkError::AppUnavailable)
            })
