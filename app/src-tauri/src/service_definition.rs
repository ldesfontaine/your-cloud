...
//! against, so a definition this App accepts is a definition that Controller
...
//! the freeze travels by the ordinary session of the App. The consent frame
...
/// The prefix of the account a slug derives, spelled here so the App can
/// display the name before anything creates it.
///
/// It is the contract's derivation and the Auxiliary's, mirrored for display
/// only: this App never sends an account name anywhere, no request of the
...
/// a function of the document, so the App reads it off the very field a
/// human is filling in.
...
///
/// It carries no `schema_version`: an App writes the one version it knows,
...
/// the contract refuses: the freeze of this App is written against the
...
/// definition this App rehashed, so a Controller that altered one byte of a
/// frozen revision is caught by the App before it is displayed and not only
...
/// One frozen revision as the App displays it: the bytes, the digest, the
...
/// App projects which machine runs which revision, so a field for it would
...
mod tests {
    ...
    /// implementations of the encoding pin, so an App that displayed one
