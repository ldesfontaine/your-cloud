...
const SNAPSHOT_FILE: &str = "app.stronghold";
const CLIENT_PATH: &[u8] = b"your-cloud-app/v1";
const DOCUMENT_KEY: &[u8] = b"app-document/v1";
...
impl AppCore {
    ...
    pub fn status(&mut self) -> Result<AppStatus, VaultError> {
    ...
    pub fn unlock(&mut self, phrase: String) -> Result<AppStatus, VaultError> {
    ...
    pub fn confirm_phrase_change(
        ...
        let temporary = self
            .directory
            .join(format!(".app-{}.stronghold", random_identifier()));
        unlocked
            .stronghold
...
fn create_snapshot(
    ...
    let client = stronghold
        ...
        .map_err(|_| VaultError::Unavailable)?;
    let document = AppDocument::empty();
    ...
    let temporary = directory.join(format!(".app-{}.stronghold", random_identifier()));
    stronghold
        .commit_with_keyprovider(
            &SnapshotPath::from_path(&temporary),
            &key_provider(key.clone())?,
...
fn commit_document(
    ...
    let verified = verifier
        ...
        .ok()
        .flatten()
        .and_then(|bytes| serde_json::from_slice::<AppDocument>(&bytes).ok())
...
fn snapshot_file_name() -> String {
    format!("app-{}.stronghold", random_identifier())
}
...
mod tests {
    ...
    /// qui, ayant volé la phrase, ouvre le coffre exactement comme l'App
    /// l'ouvre — même `AppCore`, même Argon2id, même Stronghold. Il ne
    ...
    /// App frappe, et c'est cette abstention qui fait que le refus démontre
    ...
    fn attacker_reads_the_device_credential_from_a_stolen_phrase() {
        ...
        let mut core = AppCore::new(PathBuf::from(state));
        core.unlock(phrase)
        ...
        // l'intercepteur présente à l'App le certificat que le Controller
    ...
    fn initialization_is_confirmed_before_durable_state_exists() {
        ...
        let mut core = AppCore::new(state_path.clone());
