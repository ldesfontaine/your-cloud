...
if (
  ...
  throw new Error(
    "usage: node tools/build-linux-candidate-manifest.mjs ARTIFACT APP_SBOM HELPER_SBOM OUTPUT SIGNER_FINGERPRINT GIT_HEAD",
  );
}
...
const appSbom = resolve(appSbomInput);
const helperSbom = resolve(helperSbomInput);
const output = resolve(outputInput);
if (appSbom === helperSbom) {
  throw new Error("the App and native assistant must use two distinct SBOM files");
}
...
async function collectFiles(path, collected = []) {
  ...
  if (metadata.isSymbolicLink()) {
    throw new Error(`source provenance refuses symbolic links: ${relative(appRoot, path)}`);
  }
  if (metadata.isFile()) {
    collected.push(path);
    return collected;
  ...
  if (!metadata.isDirectory()) {
    throw new Error(`source provenance refuses special files: ${relative(appRoot, path)}`);
  }
...
async function treeDigest(files) {
  ...
  for (const path of [...files].sort((left, right) => left.localeCompare(right))) {
    tree.update(`${relative(appRoot, path)}\0${await digest(path)}\n`, "utf8");
  }
...
for (const sourceRoot of sourceRoots) {
  await collectFiles(resolve(appRoot, sourceRoot), sourceFiles);
}
...
for (const helperSourceRoot of [
  ...
  await collectFiles(resolve(appRoot, helperSourceRoot), helperSourceFiles);
}
...
const inspectedAppSbom = await inspectSbom(
  appSbom,
  "your-cloud-app",
  "app",
);
...
if (helperForbiddenComponents.length > 0) {
  throw new Error(
    `helper SBOM contains forbidden App/WebKit components: ${helperForbiddenComponents.join(", ")}`,
  );
}
...
try {
  ...
  helperEvidence = {
    ...
    source_external_bin: relative(appRoot, preparedHelper),
    target,
    format: "elf",
...
const manifest = {
  schema_version: 2,
  kind: "your-cloud-app-linux-candidate",
  ...
  source: {
    ...
    package_lock_sha256: await digest(resolve(appRoot, "package-lock.json")),
    cargo_lock_sha256: await digest(resolve(appRoot, "src-tauri/Cargo.lock")),
  },
  artifacts: [
    {
  ...
  sboms: [inspectedAppSbom.evidence, inspectedHelperSbom.evidence],
  signing: {
  ...
  tools: {
    ...
    tauri_cli: command(resolve(appRoot, "node_modules/.bin/tauri"), ["--version"]),
