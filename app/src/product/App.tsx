...
export function App() {
  ...
  const selectInfrastructure = useCallback(
    async (association: AssociationSummary) => {
      await nativeApp.cancelPendingRequests().catch(() => undefined);
  ...
  const loadSelected = useCallback(async () => {
    ...
    try {
      ...
      const [nextInfrastructure, nextMachines, nextExternal] = await Promise.all([
        nativeApp.readInfrastructure(selectedInfrastructure),
        nativeApp.readMachines(selectedInfrastructure),
        nativeApp.readExternalElements(selectedInfrastructure),
      ]);
  ...
  const initializeSelected = useCallback(
    async (label: string) => {
      ...
      setFailure(null);
      try {
        const next = await nativeApp.putInfrastructure(selectedInfrastructure, label);
  ...
  const putSelectedMachine = useCallback(
    async (machineId: string, label: string) => {
      ...
      setFailure(null);
      try {
        await nativeApp.putMachine(selectedInfrastructure, machineId, label);
        await loadSelected();
        return true;
      } catch (error: unknown) {
        setFailure(operationErrorMessage(error));
  ...
  const withdrawExternalElement = useCallback(
    async (elementId: string) => {
      ...
      setFailure(null);
      try {
        await nativeApp.withdrawExternalElement(selectedInfrastructure, elementId);
        await loadSelected();
        return true;
      } catch (error: unknown) {
        setFailure(operationErrorMessage(error));
  ...
  // contre laquelle l'App tient son parc, et une lecture qui les
  ...
  const loadDispatches = useCallback(async () => {
    if (!selectedInfrastructure) return;
    try {
      const next = await nativeApp.readPlanDispatches(selectedInfrastructure);
  ...
  const loadDefinitions = useCallback(async () => {
    ...
    setFailure(null);
    try {
      const next = await nativeApp.readServiceDefinitions(selectedInfrastructure);
      if (generation !== requestGeneration.current) return;
      setDefinitions(next);
      setLoadState("ready");
    } catch (error: unknown) {
  ...
  const rotateSelectedDevice = useCallback(async () => {
    ...
    setFailure(null);
    try {
      const updated = await nativeApp.rotateDevice(selectedInfrastructure);
      setStatus((current) =>
        current
          ? {
              ...current,
  ...
  if (loadState === "loading" && !status) {
    return (
      <main className="yc-access yc-stack" aria-labelledby="startup-title">
        ...
        <LoadingBlock label="Initialisation de l'App" />
      </main>
    );
  }
    ...
    <div className="yc-app">
      <aside className="yc-sidebar">
        ...
        <div className="yc-stack">
          ...
          <Button icon={LockKeyhole} onClick={() => void lockApp()}>
            Verrouiller
          </Button>
        </div>
      </aside>
        ...
        <div className="yc-main__inner">
          ...
          {view === "profile" ? (
            <ProfileView
              ...
              onLogout={async () => {
                if (!selectedInfrastructure) return;
                await nativeApp.logout(selectedInfrastructure);
                await lockApp();
              }}
            />
          ) : null}
