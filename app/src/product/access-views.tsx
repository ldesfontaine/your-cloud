...
export function LocalAccessView({
  ...
  async function prepare() {
    ...
    try {
      if (generated) {
        await nativeApp.discardPreparation(generated.generation_id);
      }
      setGenerated(await nativeApp.prepare());
  ...
  async function confirmInitialization() {
    ...
    try {
      onStatus(
        await nativeApp.confirmInitialization(
  ...
  async function unlock() {
    ...
    onFailure(null);
    try {
      onStatus(await nativeApp.unlock(submitted));
    } catch (error: unknown) {
      onFailure(operationErrorMessage(error));
    } finally {
      setBusy(false);
...
export function AssociationView({
  ...
  async function submit() {
    ...
    setFailure(null);
    try {
      onComplete(await nativeApp.pair(input));
