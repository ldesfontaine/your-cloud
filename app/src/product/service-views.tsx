...
export function ServicesView({
  ...
  const prefill = useCallback(async () => {
    setFailure(null);
    try {
      const paste = await nativeApp.parseServiceDefinitionPaste(pasted);
  ...
  const freeze = useCallback(async () => {
    ...
    setFailure(null);
    try {
      await nativeApp.freezeServiceDefinition(
        infrastructureId,
    ...
    <Card>
      <div className="yc-stack">
        ...
        {/* Les octets exacts, affichés entiers. La borne de 8192 octets existe
            en partie pour cela : l'App montre toujours le document entier,
  ...
  if (byMachine.size === 0) {
    return (
      <Banner icon={Info} title="Instances" tone="accent">
        <p>
          Aucun plan portant ce nom n’a été lancé depuis cette App. Une définition gelée n’a
...
// la sienne : l'App ne rend jamais un code au visage d'un humain, et un nom
