...
export function PlansView({
  ...
  // juste. Le contrat dit que cette App apprend la position par la vue des
  ...
  const loadDispatches = useCallback(async () => {
    try {
      const view = await nativeApp.readPlanDispatches(infrastructureId);
  ...
  const readPair = useCallback(async () => {
    ...
    try {
      ...
      const [fleet, launched] = await Promise.all([
        nativeApp.readMachines(infrastructureId),
        nativeApp.readPlanDispatches(infrastructureId),
      ]);
      ...
      const presentation = await nativeApp.readPlanPair(
        infrastructureId,
        machineId,
        "deploy_user_service",
        slug,
  ...
  const openConsent = useCallback(async () => {
    ...
    setFailureDetail(null);
    try {
      setSession(await nativeApp.openPlanConsent(infrastructureId));
    } catch (error: unknown) {
      setFailure(operationErrorMessage(error));
      setFailureDetail(operationErrorDetail(error));
    }
  ...
  const cancelConsent = useCallback(async () => {
    if (!session) return;
    try {
      await nativeApp.cancelPlanConsent(session.request_id);
    } catch (error: unknown) {
      setFailure(operationErrorMessage(error));
      setFailureDetail(operationErrorDetail(error));
    }
  ...
  const submitDecision = useCallback(async () => {
    ...
    setBusy(true);
    try {
      await nativeApp.submitPlanDecision(
        infrastructureId,
        session.request_id,
        target.epoch,
        target.reported + 1,
    ...
    <div className="yc-stack">
      ...
      <Card>
        ...
        <p className="yc-prose">
          ...
          Controller construit la paire, et cette App n’en montre que des
            ...
            <div className="yc-plan-session">
              {session.state === "open" ? (
                <>
                  ...
                  <p className="yc-prose">
                    Répondez dans la fenêtre séparée. Cette App ne peut pas
      ...
      <Card>
        ...
        {dispatches.length === 0 ? (
          <p className="yc-prose">
            Aucun plan n’a encore été lancé depuis cette App.
          </p>
        ) : (
