...
function PhraseChangePanel() {
  ...
  async function prepare() {
    ...
    setCompleted(false);
    try {
      setPrepared(await nativeApp.preparePhraseChange());
      setCurrentPhrase("");
      setConfirmation("");
    } catch (error: unknown) {
  ...
  async function confirm() {
    ...
    setFailure(null);
    try {
      await nativeApp.confirmPhraseChange(prepared.generation_id, submittedCurrent, submittedNext);
      setPrepared(null);
      setCompleted(true);
    } catch (error: unknown) {
      setFailure(operationErrorMessage(error));
...
function RecoveryRotationPanel({
  ...
  async function prepare() {
    ...
    setFailure(null);
    try {
      setPrepared(await nativeApp.prepareRecoveryKeyRotation());
      setConfirmation("");
  ...
  async function confirm() {
    ...
    setFailure(null);
    try {
      const next = await nativeApp.confirmRecoveryKeyRotation(
  ...
  async function resume() {
    ...
    setFailure(null);
    try {
      const next = await nativeApp.resumeRecoveryKeyRotation(oldCode, newCode);
      onProgress(next);
      setOldCode("");
      setNewCode("");
    } catch (error: unknown) {
  ...
  async function finish() {
    ...
    setFailure(null);
    try {
      await nativeApp.completeRecoveryKeyRotation();
      onProgress(null);
    } catch (error: unknown) {
      setFailure(operationErrorMessage(error));
    } finally {
