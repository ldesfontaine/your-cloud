...
/// cœur émettait réellement étaient rendus à l’humain comme « l'App ne
...
const nativeErrorCodes = [
  ...
  // exactement ce qu’aucun d’eux ne pouvait dire tant que l'App ne les
  // connaissait pas.
...
function toNativeError(value: unknown): NativeOperationError {
  ...
  return new NativeOperationError("app_unavailable");
}
...
export const nativeApp = {
  status: () => namedOperation<AppStatus>("app_status"),
  prepare: () => namedOperation<GeneratedLocalSecrets>("prepare_app"),
  discardPreparation: (generationId: string) =>
    namedOperation<void>("discard_app_preparation", { generationId }),
  confirmInitialization: (
    ...
    namedOperation<AppStatus>("confirm_app_initialization", {
      generationId,
      unlockPhrase,
      recoveryCode,
      confirmedCopies,
    }),
  unlock: (phrase: string) => namedOperation<AppStatus>("unlock_app", { phrase }),
  ...
  lock: () => namedOperation<void>("lock_app"),
  ...
  // miroir Rust, qui est la même grammaire que celle du Controller. Une App
  ...
  // Lire une paire gelée. L'App n’assemble rien : elle nomme une machine,
...
export function localErrorMessage(code: NativeErrorCode): string {
  switch (code) {
    case "app_locked":
      return "Déverrouillez l'App pour continuer.";
    case "invalid_input":
    ...
    case "foreign_infrastructure":
      return "Ce plan nomme une autre infrastructure que celle à laquelle cette App est associée.";
    ...
    case "approval_signature_invalid":
      return "Le Controller n’a pas reconnu la signature de cette approbation. Rien n’a été lancé. Approuvez de nouveau ; si le refus se répète, l’identité de cette App doit être renouvelée.";
    ...
    case "app_unavailable":
      return "L'App ne peut pas terminer cette opération.";
  }
}
