...
L'App est l’application installée sur l’appareil de l’administrateur. Elle
...
## Installer et retirer l'App sous Linux
...
- **`dpkg --remove` retire déjà tout ce que le paquet a posé.** `purge` n’a rien
  ...
  effacer : l'App n’écrit rien sous `/etc` ni sous `/var`, jamais ;
- **`apt purge your-cloud` échoue** — `Unable to locate package`. Ce n’est pas un
  ...
  App s’installe depuis un fichier. Le retrait se fait donc avec `dpkg`.
...
**Le retrait ne touche pas au dossier de l'App dans le foyer de
l’humain**, et c’est voulu. On y trouve, après désinstallation :

```text
~/.local/share/fr.your-cloud.app/
  native-vault/                      (0700)
    app-<identifiant>.stronghold (0600)  le coffre chiffré
...
App : les opérations natives seraient absentes. La preuve réelle construit
