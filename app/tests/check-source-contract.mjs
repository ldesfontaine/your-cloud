...
const packageDocument = JSON.parse(await readFile(join(appRoot, "package.json"), "utf8"));
...
const cargoManifest = await readSourceText(join(appRoot, "src-tauri", "Cargo.toml"));
const cargoLock = await readSourceText(join(appRoot, "src-tauri", "Cargo.lock"));
const bootstrapRuntime = await readSourceText(
  join(appRoot, "src-tauri", "src", "bootstrap.rs"),
);
const nativeAssistantRuntime = await readSourceText(
  join(appRoot, "src-tauri", "src", "native_helper.rs"),
);
const nativeAssistantWindows = await readSourceText(
  join(appRoot, "src-tauri", "src", "native_helper", "windows.rs"),
);
...
const bootstrapProtocol = await readSourceText(
  join(appRoot, "src-tauri", "crates", "bootstrap-protocol", "src", "lib.rs"),
);
const approvalProtocol = await readSourceText(
  join(appRoot, "src-tauri", "crates", "bootstrap-protocol", "src", "approval.rs"),
);
const planProtocol = await readSourceText(
  join(appRoot, "src-tauri", "crates", "bootstrap-protocol", "src", "plan.rs"),
);
const approvalConsentProtocol = await readSourceText(
  join(appRoot, "src-tauri", "crates", "bootstrap-protocol", "src", "approval_consent.rs"),
);
const approvalRuntime = await readSourceText(join(appRoot, "src-tauri", "src", "approval.rs"));
const planV2Protocol = await readSourceText(
  join(appRoot, "src-tauri", "crates", "bootstrap-protocol", "src", "plan_v2.rs"),
);
const probePlanRuntime = await readSourceText(
  join(appRoot, "src-tauri", "src", "probe_plan.rs"),
);
const publicationPlanRuntime = await readSourceText(
  join(appRoot, "src-tauri", "src", "publication_plan.rs"),
);
const planV3Protocol = await readSourceText(
  join(appRoot, "src-tauri", "crates", "bootstrap-protocol", "src", "plan_v3.rs"),
);
const linkPlanRuntime = await readSourceText(join(appRoot, "src-tauri", "src", "link_plan.rs"));
const definitionMirror = await readSourceText(
  join(appRoot, "src-tauri", "crates", "bootstrap-protocol", "src", "service_definition.rs"),
);
const definitionRuntime = await readSourceText(
  join(appRoot, "src-tauri", "src", "service_definition.rs"),
);
const bootstrapProtocolManifest = await readSourceText(
  join(appRoot, "src-tauri", "crates", "bootstrap-protocol", "Cargo.toml"),
);
const nativeAssistantManifest = await readSourceText(
  join(appRoot, "src-tauri", "crates", "native-bootstrap-assistant", "Cargo.toml"),
);
const nativeAssistantBuild = await readSourceText(
  join(appRoot, "tools", "prepare-native-bootstrap-assistant.mjs"),
);
const nativeAssistantGate = await readSourceText(
  join(appRoot, "tools", "lib", "native-bootstrap-assistant.mjs"),
);
const sbomBuilder = await readSourceText(join(appRoot, "tools", "build-sbom.mjs"));
const candidateManifestBuilder = await readSourceText(
  join(appRoot, "tools", "build-linux-candidate-manifest.mjs"),
);
...
const linuxInstalledProof = await readSourceText(
  join(appRoot, "..", "tests", "checks", "app-linux-ci"),
);
const windowsInstalledProof = await readSourceText(
  join(appRoot, "..", "tests", "checks", "app-windows-ci.ps1"),
);
const installedUiProof = await readSourceText(
  join(appRoot, "..", "tests", "checks", "app-windows-ui-proof.py"),
);
const installedUiRasterContract = await readSourceText(
  join(appRoot, "..", "tests", "checks", "test-app-ui-raster.py"),
);
const genericSourceGate = await readSourceText(
  join(appRoot, "..", "tests", "checks", "source-v0.0.1"),
);
const appRuntime = await readSourceText(join(appRoot, "src-tauri", "src", "lib.rs"));
const networkRuntime = await readSourceText(join(appRoot, "src-tauri", "src", "network.rs"));
const productModels = await readSourceText(join(appRoot, "src", "product", "models.ts"));
const nativeOperations = await readSourceText(join(appRoot, "src", "product", "native.ts"));
const infrastructureViews = await readSourceText(
  join(appRoot, "src", "product", "infrastructure-views.tsx"),
);
const serviceViews = await readSourceText(join(appRoot, "src", "product", "service-views.tsx"));
const productApp = await readSourceText(join(appRoot, "src", "product", "App.tsx"));
const reflowFixture = await readSourceText(
  join(appRoot, "tests", "reflow", "tauri-core.ts"),
);
const reflowOracle = await readSourceText(
  join(appRoot, "..", "tests", "lab", "v0.1.0", "app-reflow", "reflow-oracle.py"),
);
const productScreens = await readSourceText(join(appRoot, "src", "product", "screens.css"));
const plansView = await readSourceText(join(appRoot, "src", "product", "plans-view.tsx"));
...
// divergerait serait celui que personne ne relit avant le tag (#55).
{
  ...
  const observation = await readSourceText(
    join(appRoot, "..", "internal", "observation", "observation.go"),
  );
  ...
  for (const path of await filesBelow(join(appRoot, "..", "internal"))) {
    ...
    if (/DaemonVersion:\s*"|"daemon_version"\s*:\s*"v\d/u.test(contents)) {
      failures.push(
        `${relative(join(appRoot, ".."), path)}: la version du producteur se lit sur observation.DaemonVersion, jamais en dur`,
      );
    }
  }
}
...
if (
  ...
  JSON.stringify({ "/usr/lib/your-cloud-app/server-bundle": "server-bundle" })
) {
  failures.push(
    "tauri.conf.json: le paquet Debian doit livrer exactement le lot serveur préparé sous /usr/lib/your-cloud-app/server-bundle",
  );
}
...
// 20 août : la phrase du palier rendue deux fois pour une unité qui retombait.
{
  const controllerUnit = await readFile(
    join(appRoot, "..", "packaging", "server-bundle", "units", "your-cloud-controller.service"),
    "utf8",
  );
...
if (
  ...
  !continuousIntegration.includes(
    "app_parent_keeps_the_gtk_helper_bounded_until_cancelled",
  ) ||
  !continuousIntegration.includes(
  ...
  !continuousIntegration.includes("app-linux-webkitgtk-smoke") ||
  !continuousIntegration.includes(
...
for (const [name, source, fragments] of [
  [
    ...
    [
      ...
      "installed native helper accepted a direct non-App parent",
      ...
      "Linux WebDriver, App, helper or loopback listeners remained after cleanup",
    ],
  ],
  [
...
if (
  !genericSourceGate.includes("tests/checks/test-app-ui-raster.py") ||
  !genericSourceGate.includes("python3 -B tests/checks/test-app-ui-raster.py")
) {
...
for (const expectedWorkspaceEntry of [
  ...
  if (!cargoManifest.includes(expectedWorkspaceEntry)) {
    failures.push(`Cargo.toml App: membre workspace absent (${expectedWorkspaceEntry})`);
  }
}
if (
  ...
  failures.push("protocole amorçage: l'App doit consommer et réexporter le crate partagé");
}
if (
...
for (const command of ["start_bootstrap", "bootstrap_status", "cancel_bootstrap"]) {
  if (!nativeOperations.includes(`"${command}"`) || !appRuntime.includes(command)) {
...
// de les appeler rougit ici, pas dans une preuve LAB trois issues plus tard.
{
  ...
  for (const caller of [
    "nativeApp.startBootstrap(",
    "nativeApp.bootstrapStatus(",
    "nativeApp.cancelBootstrap(",
  ]) {
...
const invokeHandler = appRuntime.match(
...
//
// Un élément externe est une chose que ce produit n'a pas posée : l'App la
...
// de ce qu'est un élément externe, et une App qui les lirait offrirait une
...
// Un mot que cette App ne connaît pas est un refus, jamais un rendu de
...
for (const expected of [
  ...
  "fn external_readings_refuse_a_word_this_app_does_not_know()",
...
const externalCommands = [
  ...appRuntime.matchAll(/#\[tauri::command\]\nfn (?<name>\w*external\w*)\(/gu),
...
// annoncée : l'App n'a pas d'horloge d'autorité et n'en invente pas une.
...
// ne doit pas déplacer la révision contre laquelle l'App tient son parc.
if (
  !productApp.includes(
...
// L'App n'ouvre pas la fenêtre native elle-même et ne répond jamais à sa
...
// Chaque octet affiché est un octet que cette App a rehaché : le Controller
...
// écrit pas. Une App qui composerait ses propres conséquences pourrait en
// oublier une.
...
// La date d’un gel est celle que le Controller a frappée : l'App n’a pas
...
// L'App ne possède pas de ré-encodeur canonique faisant autorité : elle
...
// L'App ne possède pas plus de ré-encodeur canonique au schéma 2 qu'au
...
// L'App ne possède pas plus de ré-encodeur canonique au schéma 3 qu'aux
...
const rawRequestCommands = ["start_bootstrap", "bootstrap_status", "cancel_bootstrap"].filter(
  (command) => {
    const commandBody = appRuntime.match(
      new RegExp(
        `#\\[tauri::command\\]\\nfn ${command}\\((?<body>[\\s\\S]*?)\\n\\}`,
        "u",
      ),
...
if (rawRequestCommands.length > 0 || !appRuntime.includes("InvokeBody::Raw(_)")) {
  failures.push(
...
if (
  !appRuntime.includes("tauri::WindowEvent::CloseRequested") ||
  !appRuntime.includes("tauri::WindowEvent::Destroyed") ||
  !appRuntime.includes("bootstrap.close()")
) {
  failures.push("lib.rs: la fermeture native ne rend pas l’état d’amorçage terminal");
}
if (
  !appRuntime.includes("struct AppLocalState") ||
  !appRuntime.includes("core: AppCore") ||
  !appRuntime.includes("bootstrap: BootstrapState") ||
  !appRuntime.includes("native_helper: NativeHelperSupervisor")
) {
  failures.push(
    "lib.rs: coffre, amorçage et helper doivent partager la même transition atomique",
  );
}
const lockCommand = appRuntime.match(
  /#\[tauri::command\]\nfn lock_app\((?<body>[\s\S]*?)\n\}/u,
)?.groups?.body;
...
if (/Command::new|std::process::Command/iu.test(appRuntime)) {
...
for (const path of await filesBelow(join(appRoot, "src-tauri", "capabilities"))) {
  const contents = await readFile(path, "utf8");
  if (/tauri[-_]plugin[-_]shell|["']shell:/iu.test(contents)) {
    failures.push(`${relative(appRoot, path)}: permission shell interdite`);
  }
}
...
//     comme « l'App ne peut pas terminer cette opération » ;
...
// aussi — un code que seule une suite construit n'est pas un code émis.
{
  ...
  for (const path of await filesBelow(join(appRoot, "src-tauri", "src"))) {
