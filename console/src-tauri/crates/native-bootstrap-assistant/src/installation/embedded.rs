...
//! des trois fichiers du lot serveur que le paquet de la Console livre, puis
...
/// Le sous-arbre du préfixe d'installation (`/usr`) où le paquet de la Console
...
const CARRIED_DIRECTORY_UNDER_PREFIX: &str = "lib/your-cloud-console/server-bundle";

/// Les noms fixes des trois fichiers, exactement ceux que la préparation du
/// paquet de la Console dépose.
...
mod tests {
    ...
    fn the_attested_position_yields_the_carried_tree() {
        ...
        assert_eq!(
            location.manifest(),
            Path::new("/usr/lib/your-cloud-console/server-bundle/bundle-manifest.json")
        );
        assert_eq!(
            location.signature(),
            Path::new("/usr/lib/your-cloud-console/server-bundle/bundle-manifest.sig")
        );
        assert_eq!(
            location.artifact(),
            Path::new("/usr/lib/your-cloud-console/server-bundle/your-cloud-server.deb")
        );
    }
