...
//! the Console core verifies the pair, but the core renders through a WebView
...
//! — the sentences the Console core derived from the two documents it had
...
//! The Console core then holds those two values against the two it computed
...
//! presentation into a bounded document and back; the Console core is the only
...
/// Largest outcome frame the Console core reads back. The outcome is a version,
...
impl ApprovalConsentOutcomeKind {
    ...
    ///
    /// Zero belongs to [`Self::Confirmed`] and to nothing else. The Console
...
/// What the window decided, as the Console core reads it once the document has
/// been validated.
///
...
pub struct ApprovalConsentV1 {
    ...
    /// Correlates this window with the one request the Console core opened, and
    ...
    ///
    /// They come from the presentation the Console core built out of the two
...
impl ApprovalConsentV1 {
    ...
    /// Console core refuses it on that ground.
...
mod tests {
    ...
    fn wire_variants_are_fixed_and_the_documents_are_closed() {
        ...
        // The two documents, pinned whole. The window and the Console core are
    ...
    /// An outcome naming a digest that is not one is refused before the Console
